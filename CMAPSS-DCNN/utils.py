...
def condition_scaler(df_train, df_test, sensor_names):
    ...
    for condition in df_train['op_cond'].unique():
        ...
        train_condition_data = df_train.loc[df_train['op_cond'] == condition, sensor_names]
        test_condition_data = df_test.loc[df_test['op_cond'] == condition, sensor_names]
        ...
        # 4. 对训练集进行标准化
        df_train.loc[df_train['op_cond'] == condition, sensor_names] = (train_condition_data - mean) / std
        
        # 5. 使用训练集的均值和标准差，对测试集进行标准化
        df_test.loc[df_test['op_cond'] == condition, sensor_names] = (test_condition_data - mean) / std
...
def get_data(dataset, sensors, sequence_length, alpha, threshold):
    ...
    # data readout
	train = pd.read_csv((dir_path+train_file), sep=r'\s+', header=None, dtype=np.float32,
					 names=col_names)
	test = pd.read_csv((dir_path+test_file), sep=r'\s+', header=None, dtype=np.float32,
					 names=col_names)
...
if __name__ == '__main__':
	...
	alpha = 0.1
	threshold = 125
	x_train, y_train, x_val, y_val, x_test, y_test = get_data('FD001', sensors, squence_length, alpha, threshold)
